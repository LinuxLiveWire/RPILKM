...
static ssize_t lastTime_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf) {
	return sprintf(buf, "%.2lu:%.2lu:%.2lu:%.9lu \n",
			(ts_last.tv_sec / 3600) % 24, (ts_last.tv_sec / 60) % 60,
			ts_last.tv_sec % 60, ts_last.tv_nsec);
}

/** @brief Display the time diff in the form secs.nanosecs to 9 places */
static ssize_t diffTime_show(struct kobject *kobj, struct kobj_attribute *attr,
		char *buf) {
...
static ssize_t isDebounce_store(struct kobject *kobj,
	...
	unsigned int temp;
	sscanf(buf, "%du", &temp);
	// use temp var for correct int->bool
...
* the numberPresses_show and numberPresses_store functions above. Using mode
* 0664 gives user and group read/write access, but others only read access.
* Recent kernel versions do not like write permission settings for "others".
*/
static struct kobj_attribute count_attr = __ATTR(numberPresses, 0664,
		numberPresses_show, numberPresses_store);
static struct kobj_attribute debounce_attr = __ATTR(isDebounce, 0664,
		isDebounce_show, isDebounce_store);
...
static void __exit erpi_button_exit(void){
	...
	free_irq(irqNumber, NULL);		// free the IRQ number, no *dev_id reqd
 ...
 * @param dev_id the *dev_id that is provided -- used to identify device.
